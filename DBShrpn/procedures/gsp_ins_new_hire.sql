...
IF EXISTS (SELECT * FROM DBShrpn.dbo.sysobjects WHERE name = 'gsp_ins_new_hire')
DROP PROCEDURE [dbo].[gsp_ins_new_hire]
GO
...
CREATE procedure [dbo].[gsp_ins_new_hire]
(
	@p_userid						varchar(30),
...
IF @w_trace_sw = 'Y'
INSERT INTO DBSosxp.dbo.msg SELECT 'Start gsp_ins_new_hire' AS msg_desc
...
IF  EXISTS (SELECT * FROM DBShrpn.sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[gog_employee_events_temp1]') AND type in (N'U'))
	DROP TABLE [dbo].[gog_employee_events_temp1]


CREATE TABLE [dbo].[gog_employee_events_temp1](
...
INSERT INTO DBShrpn.dbo.gog_employee_events_temp1    ---#t0
SELECT *
...
-- This section setup the default values
   SELECT	@w_employer_id   =  'USA-CO1',
			...
			@w_national_id_1   =  '332561137',
   			...
   			@w_organization_chart_name   =  'GOG-HR',
...
SELECT @max = COUNT(ID) FROM DBShrpn.dbo.gog_employee_events_temp1
...
BEGIN
	...
	SELECT  @event_id_01						=	event_id_01,
	  ...
	  FROM DBShrpn.dbo.gog_employee_events_temp1 t WHERE t.ID = @cnt
...
EXEC [DBShrpn].[dbo].[gsp_ins_hemp] @empl_id_01,
   @emp_id_01,
   @ind_idx,
   @eff_date_01,
   @first_name_01,
...
SELECT @max		= 0
SELECT @max		= COUNT(*) FROM DBShrpn.dbo.gog_msg_tbl WHERE msg_id = 'U00003'
...
--
IF  EXISTS (SELECT * FROM DBShrpn.sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[gog_message_temp_1]') AND type in (N'U'))
...
INSERT INTO DBShrpn.dbo.gog_message_temp_1
SELECT *
...
SELECT @max = COUNT(ID) FROM DBShrpn.dbo.gog_message_temp_1


WHILE (@cnt <= @max)
BEGIN

SELECT @msg_id = msg_id, @msg_p1 = msg_p1, @msg_p2 = msg_p2 FROM DBShrpn.dbo.gog_message_temp_1 t1 WHERE t1.[ID] = @cnt
...
IF @w_trace_sw = 'Y'
INSERT INTO DBSosxp.dbo.msg SELECT 'End gsp_ins_new_hire' AS msg_desc
