...
IF EXISTS (SELECT * FROM DBShrpn.dbo.sysobjects WHERE name = 'gsp_ins_status_change')
DROP PROCEDURE [dbo].[gsp_ins_status_change]
GO
...
CREATE procedure [dbo].[gsp_ins_status_change]
(
	@p_userid						varchar(30),
...
IF  EXISTS (SELECT * FROM DBShrpn.sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[gog_employee_events_temp5]') AND type in (N'U'))
...
INSERT INTO DBShrpn.dbo.gog_employee_events_temp5    ---#t0
SELECT *
...
SELECT @max = COUNT(ID) FROM DBShrpn.dbo.gog_employee_events_temp5
...
BEGIN
	...
	SELECT  @event_id_01						=	event_id_01,
	  ...
	  FROM DBShrpn.dbo.gog_employee_events_temp5 t WHERE t.ID = @cnt
...
--			 GOTO BYPASS_EMPLOYEE
			END
		ELSE         -- 20240906 Added additional editing  --
			IF @w_curr_status <> 'T'
			BEGIN
			UPDATE	DBShrpn.dbo.gog_employee_events_aud
			SET activity_status	=	'02'
		    ...
		    INSERT INTO DBShrpn.dbo.gog_msg_tbl
			SELECT 'U00055'					As msg_id,
					@emp_id_01					As msg_p1,
					@emp_id_01					As msg_p2,
					'To rehire, the current employee status must be terminated.'	As msg_desc
...
*/
			...
			EXECUTE DBShrpn.dbo.gsp_upd_hmpl_rehire	@p_emp_id	=	@emp_id_01,
			...
			EXECUTE DBShrpn.dbo.gsp_ins_hpcg_hepy @p_emp_id	=	@emp_id_01,
...
--     FROM [DBShrpn].[dbo].[pay_group] WHERE [pay_group_id] = @pay_group_id_03
		...
		IF @w_curr_status = 'A'
		BEGIN
			EXECUTE DBShrpn.dbo.gsp_upd_hmpl_inactivate	@p_emp_id	=	@emp_id_01,
				 @p_status_change_date			=	@w_status_change_date,
		...
		BEGIN
			...
			BEGIN
				...
				EXECUTE DBShrpn.dbo.gsp_upd_hmpl_terminate	@p_emp_id	=	@emp_id_01,
				 @p_status_change_date            =	@w_status_change_date,
...
--
		...
		IF @w_curr_status = 'I'
		BEGIN  --2
			EXECUTE DBShrpn.dbo.gsp_upd_hmpl_reactivate	@p_emp_id	=	@emp_id_01,
				 @p_status_change_date				=	@w_status_change_date,
...
IF  EXISTS (SELECT * FROM DBShrpn.sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[gog_message_temp_5]') AND type in (N'U'))
...
INSERT INTO DBShrpn.dbo.gog_message_temp_5
SELECT *
...
SELECT @max = COUNT(ID) FROM DBShrpn.dbo.gog_message_temp_5


WHILE (@cnt <= @max)
BEGIN

SELECT @msg_id = msg_id, @msg_p1 = msg_p1, @msg_p2 = msg_p2 FROM DBShrpn.dbo.gog_message_temp_5 t5 WHERE t5.[ID] = @cnt
