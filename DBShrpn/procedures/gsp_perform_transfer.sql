...
IF EXISTS (SELECT * FROM DBShrpn.dbo.sysobjects WHERE name = 'gsp_perform_transfer')
DROP PROCEDURE [dbo].[gsp_perform_transfer]
GO
...
CREATE procedure [dbo].[gsp_perform_transfer]
(
	@p_userid						varchar(30),
...
DECLARE	@w_empl_xfer_opt_code			char(2)
...
IF @w_trace_sw = 'Y'
INSERT INTO DBSosxp.dbo.msg SELECT 'Start Date: ' + CAST(GETDATE() AS CHAR (20)) AS msg_desc

IF @w_trace_sw = 'Y'
INSERT INTO DBSosxp.dbo.msg SELECT 'Start gsp_perform_transfer' AS msg_desc
...
IF  EXISTS (SELECT * FROM DBShrpn.sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[gog_employee_events_temp3]') AND type in (N'U'))
	DROP TABLE [dbo].[gog_employee_events_temp3]


CREATE TABLE [dbo].[gog_employee_events_temp3](
...
INSERT INTO DBShrpn.dbo.gog_employee_events_temp3    ---#t0
SELECT *
  FROM DBShrpn.dbo.gog_employee_events
...
SELECT @max = COUNT(ID) FROM DBShrpn.dbo.gog_employee_events_temp3

IF @w_trace_sw = 'Y'
INSERT INTO DBSosxp.dbo.msg SELECT 'Before_Loop_0: Beg ' + ' event_id: ' + @event_id_01 + ' emp_id: ' + @emp_id_01 + ' eff date: ' + @eff_date_01 + ' max_count: ' + CAST(@max AS CHAR(10))  AS msg_desc

DELETE DBShrpn.dbo.gog_msg_tbl
...
--
	SELECT  @event_id_01						=	event_id_01,
			...
			--	SELECT *
	  FROM DBShrpn.dbo.gog_employee_events_temp3 t
	 WHERE t.ID = @cnt
...
--
IF @w_trace_sw = 'Y'
INSERT INTO DBSosxp.dbo.msg SELECT 'Message_Override: Beg ' + ' event_id: ' + @event_id_01 + ' emp_id: ' + @emp_id_01 + ' eff date: ' + @eff_date_01 + ' count: ' + CAST(@cnt AS CHAR(10))  AS msg_desc

	IF  EXISTS (SELECT * FROM DBShrpn.dbo.gog_employee_events ee WHERE event_id_01 = '05' AND ee.emp_id_01 = @emp_id_01 AND emp_status_code_5 = 'RH')
	    SELECT @rehire_override = '1'
	ELSE
	    SELECT @rehire_override = '0'

IF @w_trace_sw = 'Y'
INSERT INTO DBSosxp.dbo.msg SELECT 'Message_Override: End ' + ' event_id: ' + @event_id_01 + ' emp_id: ' + @emp_id_01 + ' eff date: ' + @eff_date_01 + ' count: ' + CAST(@cnt AS CHAR(10)) + ' Override: ' + @rehire_override  AS msg_desc

--
-- Check to see if the employee current status is terminated and look ahead for Rehire record.
--
IF @w_trace_sw = 'Y'
INSERT INTO DBSosxp.dbo.msg SELECT 'Message_0: Beg ' + ' event_id: ' + @event_id_01 + ' emp_id: ' + @emp_id_01 + ' eff date: ' + @eff_date_01 + ' count: ' + CAST(@cnt AS CHAR(10))  AS msg_desc
	 ...
	 WHERE s.emp_id = @emp_id_01
	   AND s.status_change_date = (SELECT MAX(status_change_date) FROM DBShrpn.dbo.emp_status t WHERE t.emp_id = s.emp_id)
		...
		BEGIN
			  ...
			  BEGIN
			        IF @w_trace_sw = 'Y'
                    INSERT INTO DBSosxp.dbo.msg SELECT 'Message_0: Mid1 ' + ' event_id: ' + @event_id_01 + ' emp_id: ' + @emp_id_01 + ' eff date: ' + @eff_date_01 + ' count: ' + CAST(@cnt AS CHAR(10))  AS msg_desc

   					UPDATE DBShrpn.dbo.gog_employee_events_aud
			        ...
			        IF @w_trace_sw = 'Y'
                    INSERT INTO DBSosxp.dbo.msg SELECT 'Message_0: Mid2 ' + ' event_id: ' + @event_id_01 + ' emp_id: ' + @emp_id_01 + ' eff date: ' + @eff_date_01 + ' count: ' + CAST(@cnt AS CHAR(10))  AS msg_desc
		      END
		END

IF @w_trace_sw = 'Y'
INSERT INTO DBSosxp.dbo.msg SELECT 'Message_0: End ' + ' event_id: ' + @event_id_01 + ' emp_id: ' + @emp_id_01 + ' eff date: ' + @eff_date_01 + ' count: ' + CAST(@cnt AS CHAR(10))  AS msg_desc
...
INSERT INTO DBSosxp.dbo.msg SELECT 'Message_1: Beg ' + ' event_id: ' + @event_id_01 + ' emp_id: ' + @emp_id_01 + ' eff date: ' + @eff_date_01 + ' count: ' + CAST(@cnt AS CHAR(10))  AS msg_desc
	...
	IF	@i_emp_employment_exists	=	'Y' AND @i_eff_date > CAST(@eff_date_01 AS datetime)
		BEGIN
			 ...
			 INSERT INTO DBShrpn.dbo.gog_msg_tbl
			...
			INSERT INTO DBShrpn.dbo.gog_historical_message
			SELECT  'U00027'						As msg_id,
					'03'							As event_id,
					@emp_id_01 						As emp_id,
...
--
--	INSERT INTO [DBShrpn].[dbo].[gog_msgtbl] SELECT 'Before 1' + @emp_id_01 + 'cnt: ' + CONVERT(CHAR,@cnt) + '@i_empl_id: ' + @i_empl_id + '@empl_id_01: ' + @empl_id_01 AS [msgdesc]
			IF @i_empl_id = @empl_id_01
			BEGIN
				   IF EXISTS (SELECT emp_id_01 FROM DBShrpn.dbo.gog_employee_events WHERE emp_id_01 = @emp_id_01 and event_id_01 = '02')
				      BEGIN
				         UPDATE DBShrpn.dbo.gog_employee_events_aud
				            SET activity_status = '99'
		    ...
		    IF EXISTS(SELECT * FROM DBShrpn.dbo.employer WHERE empl_id = @empl_id_01 AND name like 'Pen%')
				BEGIN
					...
					BEGIN
							...
							SELECT  @w_fatal_error = '5'   -- 20240906 Added this line make sure processing does not continue.
					END
...
--
--
--  Check to see if the pay element effective date is greater than the transfer date.
--
--

--	Find the old entity & old tax entity & eff date

SELECT	@p_old_empl_id	=	empl_id, @old_tax_entity =	tax_entity_id, @eff_date	=	eff_date
  FROM	DBShrpn.dbo.emp_employment ee
 WHERE	emp_id = 	@emp_id_01
   AND	eff_date = (SELECT MAX(eff_date) FROM DBShrpn.dbo.emp_employment t WHERE t.emp_id = ee.emp_id)

SELECT @p_calendar_year               	=	LEFT(convert(varchar(10),@p_transfer_date,112),4)
...
BEGIN
	...
	SELECT	@w_empl_xfer_opt_code	= pay_element_type_code
	  ...
	  WHERE	pay_element_id	= @w_pay_element_id
	   ...
	   BEGIN
		    ...
		    BEGIN
		          ...
		          INSERT INTO DBShrpn.dbo.gog_msg_tbl
			      SELECT 'U00056'					  As msg_id,
					     @emp_id_01					  As msg_p1,
					     @pay_element_ctrl_grp_id_03  As msg_p2,
					    'Pay Element or Aggregate missing because the effective date is greater than transfer date.' As msg_desc
...
--
--
--  End of Checking to see if the pay element effective date is greater than the transfer date.
--
--
...
--                                             1023319   0801   080105   20210901	P	99999  5  GOG-HR   080010500	  080105T  	0801T   20210601          TRAN 	 29991231 	29991231   N   GD	XCD	  Y
--EXECUTE BAYLEAF.DBShrpn.dbo.hsp_upd_hrpn_02 '1023319','0801','080105','20210901','P','99999',5,'GOG-HR','080010500','','080105T','0801T','20210601','PRWA','TRAN','29991231','29991231','N','GD','XCD','Y'

IF @w_trace_sw = 'Y'
INSERT INTO DBSosxp.dbo.msg SELECT 'Detail_Loop: Beg DBShrpn.dbo.hsp_upd_hrpn_02_trn' + ' event_id: ' + @event_id_01 + ' emp_id: ' + @emp_id_01 + ' eff date: ' + @eff_date_01 + ' count: ' + CAST(@cnt AS CHAR(10))  AS msg_desc

EXECUTE DBShrpn.dbo.hsp_upd_hrpn_02_trn @p_emp_id,
				  @p_old_empl_id,
				  @p_new_empl_id,
				  @p_transfer_date,
				  @p_assign_to,
...
IF @w_trace_sw = 'Y'
INSERT INTO DBSosxp.dbo.msg SELECT 'Detail_Loop: Beg DBShrpy.dbo.hsp_ins_hpep_02_trn' + ' event_id: ' + @event_id_01 + ' emp_id: ' + @emp_id_01 + ' eff date: ' + @eff_date_01 + ' count: ' + CAST(@cnt AS CHAR(10))  AS msg_desc

EXECUTE DBShrpy.dbo.hsp_ins_hpep_02_trn @p_emp_id,
                  @p_old_empl_id,
				  @p_new_empl_id,
				  @p_transfer_date,
				  @p_calendar_year,
...
IF @w_trace_sw = 'Y'
INSERT INTO DBSosxp.dbo.msg SELECT 'Detail_Loop: End DBShrpy.dbo.hsp_ins_hpep_02_trn' + ' event_id: ' + @event_id_01 + ' emp_id: ' + @emp_id_01 + ' eff date: ' + @eff_date_01 + ' count: ' + CAST(@cnt AS CHAR(10))  AS msg_desc
...
IF  EXISTS (SELECT * FROM DBShrpn.sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[gog_message_temp_3]') AND type in (N'U'))
	DROP TABLE [dbo].[gog_message_temp_3]


CREATE TABLE [dbo].[gog_message_temp_3](
	[ID]							[int] IDENTITY(1,1) NOT NULL,
...
INSERT INTO DBShrpn.dbo.gog_message_temp_3
SELECT *
  FROM DBShrpn.dbo.gog_msg_tbl
 WHERE msg_id = 'U00013'

SET @cnt = 1

SELECT @max = COUNT(ID) FROM DBShrpn.dbo.gog_message_temp_3


WHILE (@cnt <= @max)
BEGIN

SELECT @msg_id = msg_id, @msg_p1 = msg_p1, @msg_p2 = msg_p2 FROM DBShrpn.dbo.gog_message_temp_3 t3 WHERE t3.[ID] = @cnt
...
IF @w_trace_sw = 'Y'
INSERT INTO DBSosxp.dbo.msg SELECT 'End gsp_perform_transfer' AS msg_desc
