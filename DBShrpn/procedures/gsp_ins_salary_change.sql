...
IF EXISTS (SELECT * FROM DBShrpn.dbo.sysobjects WHERE name = 'gsp_ins_salary_change')
   DROP PROCEDURE [dbo].[gsp_ins_salary_change]
GO
...
CREATE procedure [dbo].[gsp_ins_salary_change]
(
	@p_userid						varchar(30),
...
IF  EXISTS (SELECT * FROM DBShrpn.sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[gog_employee_events_temp2]') AND type in (N'U'))
	DROP TABLE [dbo].[gog_employee_events_temp2]


CREATE TABLE [dbo].[gog_employee_events_temp2](
...
INSERT INTO DBShrpn.dbo.gog_employee_events_temp2    ---#t0
SELECT *
...
SELECT @max = COUNT(ID) FROM DBShrpn.dbo.gog_employee_events_temp2
...
BEGIN
IF @w_trace_sw = 'Y'
INSERT INTO DBSosxp.dbo.msg SELECT 'Detail_Loop: Beg of Loop' + ' event_id: ' + @event_id_01 + ' emp_id: ' + @emp_id_01 + ' eff date: ' + @eff_date_01 + ' count: ' + CAST(@cnt AS CHAR(10))  AS msg_desc
	...
	SELECT  @event_id_01						=	event_id_01,
	  ...
	  FROM DBShrpn.dbo.gog_employee_events_temp2 t WHERE t.ID = @cnt
...
IF  EXISTS (SELECT * FROM DBShrpn.sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[gog_message_temp_2]') AND type in (N'U'))
	DROP TABLE [dbo].[gog_message_temp_2]


CREATE TABLE [dbo].[gog_message_temp_2](
	[ID]							[int] IDENTITY(1,1) NOT NULL,
...
INSERT INTO DBShrpn.dbo.gog_message_temp_2
SELECT *
...
SELECT @max = COUNT(ID) FROM DBShrpn.dbo.gog_message_temp_2


WHILE (@cnt <= @max)
BEGIN

SELECT @msg_id = msg_id, @msg_p1 = msg_p1, @msg_p2 = msg_p2 FROM DBShrpn.dbo.gog_message_temp_2 t1 WHERE t1.[ID] = @cnt
...
SELECT @msg_id = msg_id, @msg_p1 = msg_p1, @msg_p2 = msg_p2 FROM DBShrpn.dbo.gog_message_temp_2 t2 WHERE t2.[ID] = @cnt
