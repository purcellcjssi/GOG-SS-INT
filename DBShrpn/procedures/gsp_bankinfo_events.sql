...
IF EXISTS (SELECT * FROM DBShrpn.dbo.sysobjects WHERE name = 'gsp_bankinfo_events')
DROP PROCEDURE [dbo].[gsp_bankinfo_events]
GO
...
CREATE procedure [dbo].[gsp_bankinfo_events]
...
As
 Begin
	...
	-- Exec [dbo].[gsp_bankinfo_events] 'DBS'
	--WAITFOR DELAY '00:01';
   -- DECLARE @USER_ID			char(30)
   -- SELECT * FROM DBShrpn.dbo.gog_historical_message WHERE event_id = '07'
   -- SELECT * FROM DBShrpn.dbo.gog_bankinfo_events_aud t WHERE t.[event_id_01]	=  '07'
	...
	SELECT	@w_userid		=	[psc_userid]
     ...
     WHERE [psc_userid]		= @USER_ID
       AND [psc_pgm_parms]	= 'GOG_BANKINFO_EVENTS'
	...
	INSERT INTO DBShrpn.dbo.gog_bankinfo_events_aud
    SELECT
		 [event_id_01]
		,[emp_id_01]
		,[eff_date_01]
    ...
    FROM DBShrpn.dbo.gog_bankinfo_events bi
   WHERE NOT EXISTS (SELECT * FROM DBShrpn.dbo.gog_bankinfo_events_aud t
                       WHERE t.[event_id_01]	=   bi.[event_id_01]
                         AND t.[emp_id_01]		=	bi.[emp_id_01]
                         AND t.[activity_date]	=	@w_activity_date)


	IF  EXISTS (SELECT [event_id_01] FROM DBShrpn.dbo.gog_bankinfo_events WHERE [event_id_01] = '07' )
		BEGIN
			EXEC	DBShrpn.dbo.gsp_ins_bankinfo @w_userid,
					@w_batchname,
					@w_qualifier,
					@w_activity_date,
					@w_wflow_userid,
