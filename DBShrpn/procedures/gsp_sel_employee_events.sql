...
IF EXISTS (SELECT * FROM DBShrpn.dbo.sysobjects WHERE name = 'gsp_sel_employee_events')
DROP PROCEDURE [dbo].[gsp_sel_employee_events]
GO
...
CREATE procedure [dbo].[gsp_sel_employee_events]
...
As
 Begin
	...
	-- Exec [dbo].[gsp_sel_employee_events] 'DBS'
	--WAITFOR DELAY '00:01';
   -- DECLARE @USER_ID			char(30)
	...
	SELECT	@w_userid		=	[psc_userid]
     ...
     WHERE [psc_userid]		= @USER_ID
       AND [psc_pgm_parms]	= 'GOG_EMPLOYEE_EVENTS'
	...
	WHERE   batch_parameter_key = 'GOG_EMPLOYEE_EVENTS'

	--SELECT @w_inputfile,@w_wflow_userid,@activity_status

	INSERT INTO DBShrpn.dbo.gog_employee_events_aud
    SELECT
		 [event_id_01]
		,[emp_id_01]
		,[eff_date_01]
    ...
    FROM DBShrpn.dbo.gog_employee_events ee
   WHERE NOT EXISTS (SELECT * FROM DBShrpn.dbo.gog_employee_events_aud t
                       WHERE t.[event_id_01]	=   ee.[event_id_01]
                         AND t.[emp_id_01]		=	ee.[emp_id_01]
                         AND t.[activity_date]	=	@w_activity_date)


	IF  EXISTS (SELECT [event_id_01] FROM DBShrpn.dbo.gog_employee_events WHERE [event_id_01] = '01' )
		BEGIN
			EXEC	DBShrpn.dbo.gsp_ins_new_hire @w_userid,
					@w_batchname,
					@w_qualifier,
					@w_activity_date,
					@w_wflow_userid,
	...
	IF  EXISTS (SELECT [event_id_01] FROM DBShrpn.dbo.gog_employee_events WHERE [event_id_01] = '02' )
		BEGIN
			EXEC	DBShrpn.dbo.gsp_ins_salary_change @w_userid,
					@w_batchname,
					@w_qualifier,
					@w_activity_date,
					@w_wflow_userid,
	...
	IF  EXISTS (SELECT [event_id_01] FROM DBShrpn.dbo.gog_employee_events WHERE [event_id_01] = '03' )
		BEGIN
			EXEC	DBShrpn.dbo.gsp_perform_transfer @w_userid,
					@w_batchname,
					@w_qualifier,
					@w_activity_date,
					@w_wflow_userid,
	...
	IF  EXISTS (SELECT [event_id_01] FROM DBShrpn.dbo.gog_employee_events WHERE [event_id_01] = '04' )
		BEGIN
			EXEC	DBShrpn.dbo.gsp_ins_name_change @w_userid,
					@w_batchname,
					@w_qualifier,
					@w_activity_date,
					@w_wflow_userid,
	...
	IF  EXISTS (SELECT [event_id_01] FROM DBShrpn.dbo.gog_employee_events WHERE [event_id_01] = '05' )
		BEGIN
			EXEC	DBShrpn.dbo.gsp_ins_status_change @w_userid,
					@w_batchname,
					@w_qualifier,
					@w_activity_date,
					@w_wflow_userid,
	...
	IF  EXISTS (SELECT [event_id_01] FROM DBShrpn.dbo.gog_employee_events WHERE [event_id_01] = '06' )
		BEGIN
			EXEC	DBShrpn.dbo.gsp_ins_pay_element @w_userid,
					@w_batchname,
					@w_qualifier,
					@w_activity_date,
					@w_wflow_userid,
	 ...
	 DELETE [DBShrpn].[dbo].[gog_employee_events]
