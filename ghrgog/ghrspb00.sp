...
IF EXISTS (SELECT * FROM DBShrpn.dbo.sysobjects WHERE name = 'gsp_ins_bankinfo')
DROP PROCEDURE [dbo].[gsp_ins_bankinfo]
GO
...
CREATE procedure [dbo].[gsp_ins_bankinfo]
(
	@p_userid						varchar(30),
...
BEGIN
-- SELECT * FROM DBShrpn.dbo.gog_historical_message WHERE event_id = '07'
...
IF  EXISTS (SELECT * FROM DBShrpn.sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[gog_bankinfo_events_temp7]') AND type in (N'U'))
	DROP TABLE [dbo].[gog_bankinfo_events_temp7]


CREATE TABLE [dbo].[gog_bankinfo_events_temp7](
...
INSERT INTO DBShrpn.dbo.gog_bankinfo_events_temp7    ---#t0
SELECT * 
  FROM DBShrpn.dbo.gog_bankinfo_events
...
SELECT @max = COUNT(ID) 
FROM DBShrpn.dbo.gog_bankinfo_events_temp7
...
BEGIN
	...
	SELECT  @event_id_01						=	event_id_01,
	  ...
	  FROM DBShrpn.dbo.gog_bankinfo_events_temp7 t WHERE t.ID = @cnt
...
--
INSERT INTO DBSosxp.dbo.msg SELECT 'Bankinfo_Loop Beg: ' + ' event_id: ' + @event_id_01 + ' emp_id: ' + @emp_id_01 + ' empl_id_01: ' + @empl_id_01 + ' eff date: ' + @eff_date_01 + ' pay element: ' + @pay_element_desc_06 + ' count: ' + CAST(@cnt AS CHAR(10))  AS msg_desc 
...
IF NOT EXISTS (SELECT * FROM [DBShrpn].[dbo].[emp_pay_element] WHERE [emp_id] =	@emp_id_01 AND [empl_id]	= @empl_id_01 AND [pay_element_id] = @pay_element_desc_06)
   ...
   INSERT INTO DBSosxp.dbo.msg SELECT 'Bankinfo_Loop: Inside ' + ' event_id: ' 
   		UPDATE	DBShrpn.dbo.gog_bankinfo_events_aud
   ...
   END
   INSERT INTO DBSosxp.dbo.msg SELECT 'Bankinfo_Loop: Continue ' + ' event_id: ' 
--
--	This section will validate the interface data
-- 
...
IF  EXISTS (SELECT * FROM DBShrpn.sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[gog_message_temp_4]') AND type in (N'U'))
	DROP TABLE [dbo].[gog_message_temp_4]


CREATE TABLE [dbo].[gog_message_temp_4](
	[ID]							[int] IDENTITY(1,1) NOT NULL,
...
INSERT INTO DBShrpn.dbo.gog_message_temp_4	
SELECT * 
...
SELECT @max = COUNT(ID) FROM DBShrpn.dbo.gog_message_temp_4
 

WHILE (@cnt <= @max)
BEGIN

SELECT @msg_id = msg_id, @msg_p1 = msg_p1, @msg_p2 = msg_p2 FROM DBShrpn.dbo.gog_message_temp_4 t4 WHERE t4.[ID] = @cnt
