...
IF EXISTS (SELECT * FROM DBShrpn.dbo.sysobjects WHERE name = 'gsp_bank_verification_rpt')
DROP PROCEDURE [dbo].[gsp_bank_verification_rpt]
GO
...
CREATE procedure [dbo].[gsp_bank_current_data]
AS
BEGIN
-- EXEC [DBShrpn].[dbo].[gsp_bank_current_data]
...
GRANT EXECUTE on [dbo].[gsp_bank_current_data] TO public
