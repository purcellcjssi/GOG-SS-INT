...
IF EXISTS (SELECT * FROM DBShrpn.dbo.sysobjects WHERE name = 'gsp_ins_name_change')
   DROP PROCEDURE [dbo].[gsp_ins_name_change]
GO
...
CREATE procedure [dbo].[gsp_ins_name_change]
(
	@p_userid						varchar(30),
...
IF @w_trace_sw = 'Y'
INSERT INTO DBSosxp.dbo.msg SELECT 'Start gsp_ins_name_change' AS msg_desc

IF  EXISTS (SELECT * FROM DBShrpn.sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[gog_employee_events_temp4]') AND type in (N'U'))
	DROP TABLE [dbo].[gog_employee_events_temp4]


CREATE TABLE [dbo].[gog_employee_events_temp4](
...
INSERT INTO DBShrpn.dbo.gog_employee_events_temp4    ---#t0
SELECT * 
...
SELECT @max = COUNT(ID) FROM DBShrpn.dbo.gog_employee_events_temp4
...
BEGIN
	...
	SELECT  @event_id_01						=	event_id_01,
	  ...
	  FROM DBShrpn.dbo.gog_employee_events_temp4 t WHERE t.ID = @cnt
...
IF @w_trace_sw = 'Y'
INSERT INTO DBSosxp.dbo.msg SELECT 'End ggsp_ins_name_change' AS msg_desc    
